...
struct NO* balancear(struct NO* H){
  if(cor(H->dir) == RED) //nó vermelho é smp filho à esq
    H = rotacionaEsquerda(H);
  if(H->esq != NULL && cor(H->dir) == RED && cor(H->esq->esq) == RED) //filho da dir e neto da esq são vermelhos
    H = rotacionaDireita(H);
  if(cor(H->esq) == RED && cor(H->dir) == RED) //2 filhos vermelhos
    trocaCor(H);
  return H;
}
...
struct NO* insereNO(struct NO* H, int valor, int *resp){
  ...
  *resp = 1;
  ...
  if(valor == H->info) //valor duplicado
    *resp = 0;
  else{
    if(valor < H->info)
